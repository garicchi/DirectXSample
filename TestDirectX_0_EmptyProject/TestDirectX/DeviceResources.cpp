...
void DeviceResources::CreateWindowSizeDependentResources() 
{
	...
	else
	{
		...
		swapChainDesc.Width = static_cast<UINT>(m_d3dRenderTargetSize.Width); // ウィンドウのサイズと一致させます。
...
{
	// デバイスで次のことが発生して既定のアダプターが変更された場合は、D3D デバイスが有効でなくなります:
	// が作成されたか、またはこのデバイスが削除された場合です。
...
// アプリケーションが停止したときに、このメソッドを呼び出します。アプリケーションが使用するドライバーに関するヒントを提供します
// はアイドル状態になり、他のアプリで使用するために、一時バッファーを解放することができます。
...
// このメソッドは、表示デバイスのネイティブの方向と、次のものとの間での回転を決定します:
// 現在の表示方向。
DXGI_MODE_ROTATION DeviceResources::ComputeDisplayRotation()
{
	DXGI_MODE_ROTATION rotation = DXGI_MODE_ROTATION_UNSPECIFIED;

	//メモ: たとえ次の状況であっても、NativeOrientation として使用できるのは、Landscape または Portrait のどちらかのみです:
	// DisplayOrientations 列挙型には他の値があります。
